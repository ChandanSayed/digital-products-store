...
const page = async () => {
  ...
  async function getUserData() {
    ...
    return {
      userCount,
      averageValuePerUser: userCount == 0 ? 0 : (orderData._sum.pricePainInCents || 0) / userCount / 100
    };
  }

  async function getProductData() {
    const [inactiveCount, activeCount] = await Promise.all([db.product.count({ where: { isAvailableProductForPurchase: false } }), db.product.count({ where: { isAvailableProductForPurchase: true } })]);
    return {
      activeCount,
      inactiveCount
    };
  ...
  const [sellData, userData, productData] = await Promise.all([getSellData(), getUserData(), getProductData()]);
